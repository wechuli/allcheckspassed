...
export function returnChecksWithMatchingNameAndAppId(
  checks: ICheck[],
  name: string,
  appId: number
): ICheck[] | null {
  ...
  if (appId === -1) {
    ...
    checksWithNameAndAppID = checks.filter(
      (check) => regex.test(check.name) && check.app.id === appId
    );
  }
...
export function filterChecksWithMatchingNameAndAppId(
  checks: ICheck[],
  checksInputs: ICheckInput[]
) {
  let missingChecks: ICheckInput[] = [];
  let filteredChecksRaw: ICheck[] = [];
  checksInputs.forEach((checkInput) => {
    const checksWithNameAndAppId = returnChecksWithMatchingNameAndAppId(
      checks,
      checkInput.name,
      checkInput.app_id
    );
  ...
  const mostRecentChecks =
    takeMostRecentChecksForMatchingNameAndAppId(filteredChecksRaw);
  return { filteredChecks: mostRecentChecks, missingChecks };
}

export function takeMostRecentChecksForMatchingNameAndAppId(
  checks: ICheck[]
): ICheck[] {
  ...
  getUniqueAppId.forEach((appId) => {
    const checksWithMatchingAppId = checks.filter(
      (check) => check.app.id === appId
    );
    ...
    getUniqueCheckName.forEach((checkName) => {
      const checksWithMatchingName = checksWithMatchingAppId.filter(
        (check) => check.name === checkName
      );
      const mostRecentCheck = checksWithMatchingName.reduce((prev, current) =>
...
export function removeChecksWithMatchingNameAndAppId(
  checks: ICheck[],
  checksInputs: ICheckInput[]
) {
...
export function checkOneOfTheChecksInputIsEmpty(
  checksInputs1: ICheckInput[],
  checksInputs2: ICheckInput[]
): boolean {
...
export function removeDuplicateEntriesChecksInputsFromSelf(
  checksInputs: ICheckInput[]
): ICheckInput[] {
  let uniqueCheckInputs: ICheckInput[] = [];
  checksInputs.forEach((checkInput) => {
    const checkInputAlreadyExists = uniqueCheckInputs.some(
      (uniqueCheckInput) =>
        uniqueCheckInput.name === checkInput.name &&
...
export function removeDuplicateChecksEntriesFromSelf(
  checks: ICheck[]
): ICheck[] {
  // use the check id to determine uniqueness
  let uniqueChecks: ICheck[] = [];
  checks.forEach((check) => {
    const checkAlreadyExists = uniqueChecks.some(
      (uniqueCheck) => uniqueCheck.id === check.id
    );
