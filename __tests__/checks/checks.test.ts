...
import * as fileExtractor from '../../src/utils/fileExtractor';
...
jest.mock('../../src/utils/fileExtractor');
...
describe('Checks', () => {
  ...
  beforeEach(() => {
    ...
    // Mock API functions
    getAllChecksMock = jest.spyOn(checksAPI, 'getAllChecks').mockResolvedValue(mockChecks);
    extractOwnCheckNameFromWorkflowMock = jest.spyOn(fileExtractor, 'extractOwnCheckNameFromWorkflow').mockResolvedValue('allcheckspassed');
  ...
  describe('constructor', () => {
    it('should set all properties correctly', () => {
      ...
      expect(checks['treatSkippedAsPassed']).toBe(defaultProps.treatSkippedAsPassed);
      expect(checks['treatNeutralAsPassed']).toBe(defaultProps.treatNeutralAsPassed);
      expect(checks['failFast']).toBe(defaultProps.failFast);
      expect(checks['failStep']).toBe(defaultProps.failStep);
      expect(checks['failOnMissingChecks']).toBe(defaultProps.failOnMissingChecks);
  ...
  describe('fetchAllChecks', () => {
    ...
    it('should throw error if getAllChecks fails', async () => {
      ...
      const checks = new Checks(defaultProps);
      await expect(checks.fetchAllChecks()).rejects.toThrow('Error getting all checks: API Error');
    });
  });
  ...
  describe('filterChecks', () => {
    ...
    it('should warn if own check cannot be determined', async () => {
      ...
      extractOwnCheckNameFromWorkflowMock.mockResolvedValueOnce('different-check-name');
  ...
  describe('evaluateChecksStatus', () => {
    ...
    it('should handle treatSkippedAsPassed option', () => {
      ...
      const checksWithSkippedAsPassed = new Checks({ ...defaultProps, treatSkippedAsPassed: true });
      const resultWithSkippedAsPassed = checksWithSkippedAsPassed.evaluateChecksStatus([mockChecks[3]]); // SKIPPED check
      expect(resultWithSkippedAsPassed).toEqual({ in_progress: false, passed: true });
    ...
    it('should handle treatNeutralAsPassed option', () => {
      ...
      const checksWithNeutralAsPassed = new Checks({ ...defaultProps, treatNeutralAsPassed: true });
      const resultWithNeutralAsPassed = checksWithNeutralAsPassed.evaluateChecksStatus([mockChecks[4]]); // NEUTRAL check
      expect(resultWithNeutralAsPassed).toEqual({ in_progress: false, passed: true });
      
      // When treatNeutralAsPassed is false
      const checksWithNeutralAsFailed = new Checks({ ...defaultProps, treatNeutralAsPassed: false });
      const resultWithNeutralAsFailed = checksWithNeutralAsFailed.evaluateChecksStatus([mockChecks[4]]); // NEUTRAL check
      expect(resultWithNeutralAsFailed).toEqual({ in_progress: false, passed: false });
    });
  ...
  describe('iterateChecks', () => {
    it('should call fetchAllChecks and filterChecks', async () => {
      const checks = new Checks(defaultProps);
      const fetchAllChecksSpy = jest.spyOn(checks, 'fetchAllChecks').mockResolvedValue();
      const filterChecksSpy = jest.spyOn(checks, 'filterChecks').mockResolvedValue();
    ...
    it('should evaluate check statuses correctly', async () => {
      ...
      const evaluateChecksStatusSpy = jest.spyOn(checks, 'evaluateChecksStatus')
    ...
    it('should exclude own check from evaluation', async () => {
      ...
      expect(evaluateChecksStatusSpy).toHaveBeenCalledWith(
        expect.not.arrayContaining([expect.objectContaining({ id: mockOwnCheck.id })])
      );
    });
  });
  ...
  describe('run', () => {
    it('should poll for check status when poll is true', async () => {
      ...
      const iterateChecksSpy = jest.spyOn(checks, 'iterateChecks')
    ...
    it('should fail on missing checks when failOnMissingChecks and failStep are true', async () => {
      ...
      expect(setFailedMock).toHaveBeenCalledWith('Failing due to missing checks');
    });
    ...
    it('should handle polling correctly', async () => {
      ...
      jest.spyOn(checks, 'iterateChecks')
        .mockResolvedValueOnce({
          checksResult: { in_progress: true, passed: false },
          missingChecks: [],
          filteredChecksExcludingOwnCheck: []
