import {
  ...
  isValidCheckInput,
  validateCheckInputs,
} from "../src/utils/inputsExtractor";
...
describe("parseChecksArray", () => {
  ...
  it("returns an array of objects if input is a JSON array", () => {
    expect(
      parseChecksArray(
        '[{"name":"check1","app_id":1},{"name":"check2","app_id":2}]',
      ),
    ).toEqual([
      { name: "check1", app_id: 1 },
      { name: "check2", app_id: 2 },
    ]);
  });
  ...
  it("returns an array of objects if input is a JSON array omitting empty values", () => {
    expect(
      parseChecksArray(
        '[{"name":"check1","app_id":1},{"name":"","app_id":-1}]',
      ),
    ).toEqual([{ name: "check1", app_id: 1 }]);
  });

  it("returns an array of objects if input is a JSON object", () => {
    expect(parseChecksArray('{"name":"check1","app_id":1}')).toEqual([
      { name: "check1", app_id: 1 },
    ]);
  });

  it("returns an array of objects if input is a comma-separated list of check names", () => {
    expect(parseChecksArray("check1,check2")).toEqual([
      { name: "check1", app_id: -1 },
      { name: "check2", app_id: -1 },
    ]);
  });
  ...
  it("return an error if the parsed JSON does not have the correct properties", () => {
    expect(() =>
      parseChecksArray('[{"name":"check1","app_id":1},{"name":"check2"}]'),
    ).toThrow();
  });
});
...
describe("isValidCheckInput", () => {
  ...
  it("returns false if input is not a valid check input", () => {
    ...
    expect(
      isValidCheckInput({ name: "check1", app_id: 1, extra: "extra" }),
    ).toBe(true);
  });
});
...
describe("validateCheckInputs", () => {
  ...
  it("returns false if input is an array of invalid check inputs", () => {
    expect(
      validateCheckInputs([{ name: "check1", app_id: 1 }, { name: "check2" }]),
    ).toBe(false);
    expect(
      validateCheckInputs([{ name: "check1", app_id: 1 }, { app_id: 1 }]),
    ).toBe(false);
    expect(
      validateCheckInputs([
        { name: "check1", app_id: 1 },
    ...
    expect(
      validateCheckInputs([
        { name: "check1", app_id: 1 },
        { name: "check2", app_id: 2, extra: "extra" },
      ]),
    ).toBe(true);
  });
});
